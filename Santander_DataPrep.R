...
rm(dt_cor, dt_eig, dt_eiv); gc()
pca_results = prcomp(x = dt_pca, retx = TRUE, scale = TRUE, tol = 0.5)
# pca_results2 = princomp(x = dt_pca)
# pca_results3 = psych::principal(r = dt_cor, nfactors = n_facs, rotate = "varimax", scores = TRUE, n.obs = nrow(dt_pca))

# Now let's construct new data.tables:
new_train  = cbind(train[, 1:2], 
                   pca_results$x[1:nrow(train), 1:n_facs])
test_start = nrow(train)+1
test_end   = nrow(test)+nrow(train)
new_test   = cbind(test[, 1], 
                   pca_results$x[test_start:test_end, 1:n_facs])
rm(test_start, test_end)
